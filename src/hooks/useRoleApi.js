...
export default function useRoleApi(initialLimit = 100) {
  ...
  // Fetch live paginated roles & stats from backend MongoDB API
  const fetchRoles = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error fetching live user roles:", err);
      setRoles([]);
      setTotalItems(0);
      setTotalPages(1);
    } finally {
  ...
  const createRole = async (formData) => {
    ...
    try {
      const res = await axiosSecure.post("/role", formData);
      await fetchRoles();
      return res.data;
    } finally {
  ...
  const updateRole = async (id, formData) => {
    ...
    submitLockRef.current = true;
    try {
      const res = await axiosSecure.patch(`/role/${id}`, formData);
      await fetchRoles();
      return res.data;
    } finally {
      submitLockRef.current = false;
  ...
  const deleteRole = async (id) => {
    ...
    try {
      const res = await axiosSecure.delete(`/role/${id}`);
      await fetchRoles();
      return res.data;
    } finally {
