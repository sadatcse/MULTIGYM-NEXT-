...
export default function ProductCategoriesPage() {
  ...
  const validateForm = () => {
    const errors = {};
    if (!formData.title.trim()) errors.title = "Product category title is required";
    ...
    const duplicateTitle = productCategories.find(
      (c) =>
        c.title.trim().toLowerCase() === formData.title.trim().toLowerCase() && c._id !== editingCategory?._id
    );
    if (duplicateTitle) errors.title = `Product category "${formData.title}" already exists`;

    const duplicateOrder = productCategories.find(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCategory) {
        ...
        Swal.fire({ title: "Updated!", text: `Product category "${payload.title}" updated successfully.`, icon: "success", confirmButtonColor: "#FF1818", timer: 2000 });
      } else {
        await createProductCategory(payload);
        Swal.fire({ title: "Created!", text: `Product category "${payload.title}" created successfully.`, icon: "success", confirmButtonColor: "#FF1818", timer: 2000 });
      }
      setIsModalOpen(false);
    } catch (err) {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteProductCategory(deletingCategory._id);
      Swal.fire({ title: "Deleted!", text: `Product category "${deletingCategory.title}" has been deleted.`, icon: "success", confirmButtonColor: "#FF1818", timer: 2000 });
  ...
  if (!canView && !loading) {
    return (
      <div className="space-y-6 w-full max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-6 pb-10">
        <Mtitle title="Product Categories" subtitle="Manage product & item categories (Cardio, Strength, Supplements, HVAC, CCTV, etc.)." />
        <div className="bg-brand-white dark:bg-brand-charcoal p-8 rounded-3xl border border-brand-beige/50 dark:border-brand-dark-grey/50 text-center max-w-xl mx-auto shadow-xl">
          ...
          <p className="text-xs text-brand-dark-grey dark:text-brand-gold-light mb-6">
            You do not have view permission for Product Categories. Please contact your system administrator.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6 w-full max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-6 pb-10">
      <Mtitle
        ...
        subtitle="Manage product & item categories (Cardio, Strength, Supplements, HVAC, CCTV, etc.)."
        rightcontent={
          canAdd ? (
            <button
      ...
      {loading ? (
        ...
        <div className="bg-brand-white dark:bg-brand-charcoal rounded-3xl p-12 border border-brand-beige/50 dark:border-brand-dark-grey/50 text-center max-w-lg mx-auto shadow-sm">
          ...
          <h3 className="text-lg font-black text-brand-black dark:text-brand-white mb-1">No Product Categories Found</h3>
          <p className="text-xs text-brand-dark-grey dark:text-brand-gold-light mb-6">
            {searchInput || statusFilter !== "all" ? "No categories match your active filters." : "No product categories have been configured yet."}
          </p>
          {canAdd && (
                    ...
                    >
                      <div>
                        <div className="flex items-start justify-between gap-3 mb-3">
                          <div>
                            <span className="text-[10px] uppercase tracking-widest text-brand-dark-grey dark:text-brand-gold-light font-extrabold block">Product Category</span>
            ...
            >
              <div className="bg-brand-offwhite dark:bg-brand-midnight px-6 py-4 border-b border-brand-beige/60 dark:border-brand-dark-grey/60 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-base font-black text-brand-black dark:text-brand-white">{editingCategory ? "Edit Product Category" : "Add New Product Category"}</h3>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Cardio Machines & Fitness Equipment"
      ...
      <ConfirmDeleteModal
        ...
        itemName={deletingCategory?.title || "Product Category"}
