...
export default function VendorCategoriesPage() {
  ...
  const canView = can("vendor-categories", "view");
  const canAdd = can("vendor-categories", "add");
  const canEdit = can("vendor-categories", "edit");
  const canDelete = can("vendor-categories", "delete");
  ...
  const validateForm = () => {
    const errors = {};
    if (!formData.title.trim()) errors.title = "Vendor category title is required";
    ...
    const duplicateTitle = vendorCategories.find(
      (c) =>
        c.title.trim().toLowerCase() === formData.title.trim().toLowerCase() && c._id !== editingCategory?._id
    );
    if (duplicateTitle) errors.title = `Vendor category "${formData.title}" already exists`;

    const duplicateOrder = vendorCategories.find(
      (c) => Number(c.order) === Number(formData.order) && c._id !== editingCategory?._id
    );
    if (duplicateOrder) errors.order = `Order #${formData.order} is already assigned to "${duplicateOrder.title}"`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCategory) {
        await updateVendorCategory(editingCategory._id, payload);
        Swal.fire({ title: "Updated!", text: `Vendor category "${payload.title}" updated successfully.`, icon: "success", confirmButtonColor: "#FF1818", timer: 2000 });
      } else {
        await createVendorCategory(payload);
        Swal.fire({ title: "Created!", text: `Vendor category "${payload.title}" created successfully.`, icon: "success", confirmButtonColor: "#FF1818", timer: 2000 });
      }
      setIsModalOpen(false);
    } catch (err) {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteVendorCategory(deletingCategory._id);
      Swal.fire({ title: "Deleted!", text: `Vendor category "${deletingCategory.title}" has been deleted.`, icon: "success", confirmButtonColor: "#FF1818", timer: 2000 });
      setIsDeleteModalOpen(false);
    } catch (err) {
      const msg = err?.response?.data?.message || "Failed to delete vendor category.";
      Swal.fire({ title: "Error!", text: Array.isArray(msg) ? msg.join(", ") : msg, icon: "error", confirmButtonColor: "#FF1818" });
    } finally {
      setIsDeleting(false);
  ...
  if (!canView && !loading) {
    return (
      <div className="space-y-6 w-full max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-6 pb-10">
        <Mtitle title="Vendor Categories" subtitle="Manage vendor category types (ISP, CCTV, AC, IT, Security, etc.)." />
        <div className="bg-brand-white dark:bg-brand-charcoal p-8 rounded-3xl border border-brand-beige/50 dark:border-brand-dark-grey/50 text-center max-w-xl mx-auto shadow-xl">
          ...
          <p className="text-xs text-brand-dark-grey dark:text-brand-gold-light mb-6">
            You do not have view permission for Vendor Categories. Please contact your system administrator.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6 w-full max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-6 pb-10">
      <Mtitle
        ...
        subtitle="Manage vendor category types (ISP, CCTV, AC, IT, Security, etc.)."
        rightcontent={
          canAdd ? (
            <button
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <motion.div variants={itemVariants} className="bg-brand-white dark:bg-brand-charcoal p-5 rounded-3xl border border-brand-beige/50 dark:border-brand-dark-grey/50 shadow-sm hover:border-brand-gold/50 transition-all duration-300 group">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-[11px] font-extrabold uppercase tracking-wider text-brand-dark-grey dark:text-brand-gold-light block">Total Categories</span>
              <span className="text-2xl font-black text-brand-black dark:text-brand-white mt-1 block">{stats.totalCategories}</span>
            </div>
        ...
        <motion.div variants={itemVariants} className="bg-brand-white dark:bg-brand-charcoal p-5 rounded-3xl border border-brand-beige/50 dark:border-brand-dark-grey/50 shadow-sm hover:border-emerald-500/50 transition-all duration-300 group">
          <div className="flex items-center justify-between">
            <div>
              ...
              <span className="text-2xl font-black text-emerald-500 mt-1 block">{stats.activeCategories}</span>
            </div>
        ...
        <motion.div variants={itemVariants} className="bg-brand-white dark:bg-brand-charcoal p-5 rounded-3xl border border-brand-beige/50 dark:border-brand-dark-grey/50 shadow-sm hover:border-rose-500/50 transition-all duration-300 group">
          <div className="flex items-center justify-between">
            <div>
              ...
              <span className="text-2xl font-black text-rose-500 mt-1 block">{stats.inactiveCategories}</span>
            </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
          ...
          <div className="bg-brand-offwhite dark:bg-brand-midnight p-1 rounded-2xl border border-brand-beige/50 dark:border-brand-dark-grey/50 flex items-center gap-1">
            <button
              ...
              className={`p-1.5 rounded-xl transition-all cursor-pointer ${viewMode === "table" ? "bg-brand-gold text-brand-midnight shadow-xs" : "text-brand-dark-grey dark:text-brand-gold-light"}`}
              title="Table View"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-xl transition-all cursor-pointer ${viewMode === "grid" ? "bg-brand-gold text-brand-midnight shadow-xs" : "text-brand-dark-grey dark:text-brand-gold-light"}`}
      ...
      {loading ? (
        ...
        <div className="bg-brand-white dark:bg-brand-charcoal rounded-3xl p-12 border border-brand-beige/50 dark:border-brand-dark-grey/50 text-center max-w-lg mx-auto shadow-sm">
          ...
          <h3 className="text-lg font-black text-brand-black dark:text-brand-white mb-1">No Vendor Categories Found</h3>
          <p className="text-xs text-brand-dark-grey dark:text-brand-gold-light mb-6">
            {searchInput || statusFilter !== "all" ? "No categories match your active filters." : "No vendor categories have been configured yet."}
          </p>
          {canAdd && (
                      ...
                      {vendorCategories.map((category, idx) => {
                        ...
                        return (
                          <motion.tr
                            ...
                            exit="exit"
                            layout
                            className={`hover:bg-brand-gold/5 dark:hover:bg-brand-gold/10 transition-all duration-150 ${rowBusy ? "opacity-50 pointer-events-none" : ""}`}
                          >
                            <td className="py-4 px-6 text-center">
                              <span className="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-brand-gold/10 text-brand-gold font-black text-xs">#{category.order}</span>
                            </td>
                            <td className="py-4 px-6 font-extrabold text-brand-black dark:text-brand-white text-sm">{category.title}</td>
                            <td className="py-4 px-6 text-brand-dark-grey dark:text-brand-gold-light/90 font-medium max-w-xs truncate">{category.description || "—"}</td>
                            <td className="py-4 px-6 text-center">
                              <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-[11px] font-extrabold capitalize ${category.status === "active" ? "bg-emerald-500/10 text-emerald-500 border border-emerald-500/20" : "bg-rose-500/10 text-rose-500 border border-rose-500/20"}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${category.status === "active" ? "bg-emerald-500" : "bg-rose-500"}`} />
                                {category.status}
                              </span>
                            </td>
                            <td className="py-4 px-6 text-center">
                              <div className="flex items-center justify-center gap-2">
                                {canEdit && (
                                  <button onClick={() => handleOpenEdit(category)} disabled={rowBusy} className="p-2 rounded-xl text-brand-gold bg-brand-gold/10 hover:bg-brand-gold hover:text-brand-midnight transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" title="Edit Category">
                                    <FiEdit3 className="text-sm" />
                                  </button>
                                )}
                                {canDelete && (
                                  <button onClick={() => handleOpenDelete(category)} disabled={rowBusy} className="p-2 rounded-xl text-brand-red bg-brand-red/10 hover:bg-brand-red hover:text-white transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" title="Delete Category">
                ...
                {vendorCategories.map((category) => {
                  ...
                  return (
                    <motion.div
                      ...
                      exit="exit"
                      layout
                      className={`bg-brand-white dark:bg-brand-charcoal p-6 rounded-3xl border border-brand-beige/50 dark:border-brand-dark-grey/50 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between group ${rowBusy ? "opacity-50 pointer-events-none" : ""}`}
                    >
                      <div>
                        <div className="flex items-start justify-between gap-3 mb-3">
                          <div>
                            <span className="text-[10px] uppercase tracking-widest text-brand-dark-grey dark:text-brand-gold-light font-extrabold block">Category</span>
                            <h3 className="text-base font-black text-brand-black dark:text-brand-white mt-0.5">{category.title}</h3>
                          </div>
                          <span className="shrink-0 w-8 h-8 rounded-2xl bg-brand-gold/10 text-brand-gold font-black text-xs flex items-center justify-center">#{category.order}</span>
                        </div>
                        <p className="text-xs text-brand-dark-grey dark:text-brand-gold-light/90 font-medium leading-relaxed mb-4 line-clamp-2">{category.description || "No description provided."}</p>
                      </div>
            ...
            >
              <div className="bg-brand-offwhite dark:bg-brand-midnight px-6 py-4 border-b border-brand-beige/60 dark:border-brand-dark-grey/60 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-base font-black text-brand-black dark:text-brand-white">{editingCategory ? "Edit Vendor Category" : "Add New Vendor Category"}</h3>
                    <p className="text-[11px] text-brand-dark-grey dark:text-brand-gold-light font-medium">Configure title, display order, and status</p>
                  </div>
                </div>
                <button type="button" onClick={handleCloseModal} disabled={isSubmitting} title={isSubmitting ? "Please wait for the save to finish" : "Close"} className="p-1.5 rounded-xl text-brand-dark-grey hover:text-brand-black dark:hover:text-white transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 rounded-2xl text-xs font-bold bg-brand-offwhite dark:bg-brand-midnight border ${formErrors.title ? "border-brand-red focus:ring-brand-red/50" : "border-brand-beige/60 dark:border-brand-dark-grey focus:ring-brand-gold/50"} text-brand-black dark:text-brand-white outline-none focus:ring-2 disabled:opacity-60 disabled:cursor-not-allowed`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. 1"
                    className={`w-full px-4 py-2.5 rounded-2xl text-xs font-bold bg-brand-offwhite dark:bg-brand-midnight border ${formErrors.order ? "border-brand-red focus:ring-brand-red/50" : "border-brand-beige/60 dark:border-brand-dark-grey focus:ring-brand-gold/50"} text-brand-black dark:text-brand-white outline-none focus:ring-2 disabled:opacity-60 disabled:cursor-not-allowed`}
                  />
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Brief summary of this vendor category..."
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="status" value="active" checked={formData.status === "active"} disabled={isSubmitting} onChange={(e) => setFormData((prev) => ({ ...prev, status: e.target.value }))} className="w-4 h-4 accent-emerald-500 cursor-pointer disabled:cursor-not-allowed disabled:opacity-60" />
                      <span className="text-xs font-bold text-emerald-500">Active</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="status" value="inactive" checked={formData.status === "inactive"} disabled={isSubmitting} onChange={(e) => setFormData((prev) => ({ ...prev, status: e.target.value }))} className="w-4 h-4 accent-rose-500 cursor-pointer disabled:cursor-not-allowed disabled:opacity-60" />
      ...
      <ConfirmDeleteModal
        ...
        itemName={deletingCategory?.title || "Vendor Category"}
