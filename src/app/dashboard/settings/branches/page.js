...
export default function BranchesPage() {
  ...
  const validateForm = () => {
    ...
    const dupName = branches.find(
      (b) =>
        b.name.toLowerCase() === trimmedName.toLowerCase() &&
        b._id !== editingBranch?._id
    );
    if (dupName) {
      setFormError(`Branch "${trimmedName}" already exists on current page.`);
      return false;
    }

    const dupOrder = branches.find(
      (b) => b.order === Number(formData.order) && b._id !== editingBranch?._id
    );
    if (dupOrder) {
    ...
    try {
      if (editingBranch) {
        ...
        Swal.fire({
          ...
          text: `Branch "${formData.name}" has been updated.`,
          icon: "success",
        ...
        Swal.fire({
          ...
          text: `Branch "${formData.name}" added to system.`,
          icon: "success",
          confirmButtonColor: "#FF1818",
        });
      }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      Swal.fire({
        title: "Deleted!",
        text: `Branch "${deletingBranch.name}" has been removed.`,
        icon: "success",
        confirmButtonColor: "#FF1818",
      });
    } catch (err) {
  ...
  return (
    <div className="space-y-6 w-full max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-6 pb-10">
                  ...
                  <thead className="bg-brand-offwhite dark:bg-brand-midnight uppercase text-[10px] font-extrabold tracking-widest text-brand-dark-grey dark:text-brand-gold-light border-b border-brand-beige/60 dark:border-brand-dark-grey/60">
                    <tr>
                      ...
                      <th className="py-4 px-6">
                        <span className="inline-flex items-center gap-1">
                          <FiMapPin className="text-brand-gold" /> Branch Name & Address
                        </span>
                      </th>
                    ...
                    {branches.length > 0 ? (
                      <AnimatePresence initial={false}>
                        {branches.map((branch, idx) => {
                          const rowBusy = isDeleting && deletingBranch?._id === branch._id;
                          return (
                            <motion.tr
                              key={branch._id}
                              custom={idx}
                            ...
                            >
                              ...
                              <td className="py-4 px-6">
                                <div className="flex items-start gap-3">
                                  <div className="w-9 h-9 rounded-2xl bg-brand-gold/10 text-brand-gold flex items-center justify-center font-bold text-base shrink-0 group-hover:bg-brand-red group-hover:text-white transition-colors duration-200 mt-0.5">
                                    <FiMapPin />
                                  </div>
                                  <div>
                                    ...
                                    <span className="text-[11px] text-brand-dark-grey dark:text-brand-gold-light font-medium block mt-0.5 max-w-md leading-relaxed">
                                      {branch.address}
                                    </span>
                                  </div>
                                </div>
                                ...
                                <div className="space-y-1">
                                  {branch.phone && (
                                    <span className="flex items-center gap-1.5 text-xs font-bold text-brand-black dark:text-brand-white">
                                      <FiPhone className="text-brand-gold text-xs shrink-0" />
                                  ...
                                  {branch.website && (
                                    <a
                                      ...
                                      className="flex items-center gap-1.5 text-[11px] font-bold text-brand-gold hover:underline"
                                    >
                                      <FiGlobe className="text-xs shrink-0" />
                                      <span className="truncate max-w-[180px]">{branch.website}</span>
                                    </a>
                                  )}
                                </div>
                              </td>
                                ...
                                >
                                  <span
                                    className={`w-1.5 h-1.5 rounded-full ${
                                      branch.status === "active" ? "bg-emerald-500 animate-pulse" : "bg-brand-red"
                                    }`}
                                  />
                                  {branch.status}
                                </span>
                      ...
                      <tr>
                        <td colSpan="5" className="py-12 text-center text-brand-dark-grey dark:text-brand-gold-light font-medium">
                      ...
                      >
                        ...
                        <div className="space-y-2.5">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-2xl bg-brand-gold/10 text-brand-gold flex items-center justify-center text-lg font-bold group-hover:scale-110 transition-transform shrink-0">
                              <FiMapPin />
                            </div>
                          ...
                          <p className="text-xs text-brand-dark-grey dark:text-brand-gold-light leading-relaxed pl-1">
                            {branch.address}
                          </p>

                          <div className="pt-2 space-y-1 pl-1">
                            {branch.phone && (
                              <span className="flex items-center gap-2 text-xs font-bold text-brand-black dark:text-brand-white">
                                <FiPhone className="text-brand-gold text-xs" />
                                <span>{branch.phone}</span>
                              </span>
                            )}
                            {branch.website && (
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-bold uppercase tracking-wider text-brand-dark-grey dark:text-brand-gold-light mb-1.5">
                      Branch Name <span className="text-brand-red">*</span>
                    </label>
                    <input
                      type="text"
                      required
                      ...
                      placeholder="e.g. Power Fit — Adabor"
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-brand-dark-grey dark:text-brand-gold-light mb-1.5 flex items-center gap-1">
                      <FiHash className="text-brand-gold" /> Order <span className="text-brand-red">*</span>
                    </label>
                    <input
                      type="number"
                      min="1"
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-brand-dark-grey dark:text-brand-gold-light mb-1.5 flex items-center gap-1">
                    <FiMapPin className="text-brand-gold" /> Full Address <span className="text-brand-red">*</span>
                  </label>
                  <textarea
                    rows={2}
                    required
                    ...
                    placeholder="e.g. 5th & 6th Floors, 48/49 Jonota Cooperative Housing Society..."
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-brand-dark-grey dark:text-brand-gold-light mb-1.5 flex items-center gap-1">
                      <FiPhone className="text-brand-gold" /> Phone Number <span className="text-brand-red">*</span>
                    </label>
                    <input
                      type="text"
                      required
                      ...
                      placeholder="e.g. (+880) 1313-197426"
                      className="w-full px-4 py-2.5 rounded-xl text-xs bg-brand-offwhite dark:bg-brand-midnight border border-brand-beige dark:border-brand-dark-grey focus:outline-none focus:ring-2 focus:ring-brand-gold/50 text-brand-black dark:text-brand-white disabled:opacity-60 disabled:cursor-not-allowed"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-brand-dark-grey dark:text-brand-gold-light mb-1.5 flex items-center gap-1">
                      <FiGlobe className="text-brand-gold" /> Website URL
                    </label>
                    <input
                      type="text"
                      disabled={isSubmitting}
      ...
      <ConfirmDeleteModal
        ...
        itemName={deletingBranch?.name || "Branch"}
