...
export default function LatePolicyPage() {
  ...
  const totalCount = stats?.totalPolicies || latePolicies.length;
  const activeCount = stats?.activeCount ?? latePolicies.filter((p) => p.status === "active").length;
  const inactiveCount = Math.max(0, totalCount - activeCount);
  const avgTolerance = latePolicies.length
    ? Math.round(latePolicies.reduce((acc, p) => acc + (Number(p.lateTolerance) || 0), 0) / latePolicies.length)
    : 15;
  ...
  const validateForm = () => {
    ...
    const duplicateName = latePolicies.find(
      (p) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        ...
        Swal.fire({
          title: "Updated!",
          text: `Late policy "${formData.policyName}" updated successfully.`,
          icon: "success",
        ...
        Swal.fire({
          title: "Created!",
          text: `Late policy "${formData.policyName}" created successfully.`,
          icon: "success",
          confirmButtonColor: "#FF1818",
          timer: 2000,
        });
      ...
      const msg = err?.response?.data?.message || "Failed to save late policy.";
      Swal.fire({
        title: "Error!",
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteLatePolicy(deletingItem._id);
      Swal.fire({
        title: "Deleted!",
        text: `Late policy "${deletingItem.policyName}" has been deleted.`,
        icon: "success",
        confirmButtonColor: "#FF1818",
        timer: 2000,
      });
      ...
      const msg = err?.response?.data?.message || "Failed to delete late policy.";
      Swal.fire({
        title: "Error!",
        text: Array.isArray(msg) ? msg.join(", ") : msg,
        icon: "error",
    ...
    return (
      <div className="space-y-6 w-full max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-6 pb-10 font-sans">
        <Mtitle
          ...
          subtitle="Configure attendance rules, grace times, deduction rates, monthly limits, and compensated work hour rules."
        />
        <div className="bg-brand-white dark:bg-brand-charcoal p-8 rounded-3xl border border-brand-beige/50 dark:border-brand-dark-grey/50 text-center max-w-xl mx-auto shadow-xl">
          ...
          <p className="text-xs text-brand-dark-grey dark:text-brand-gold-light mb-6">
            You do not have view permission for Late Policy. Please contact your system administrator.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6 w-full max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-6 pb-10 font-sans">
        ...
        <div className="bg-brand-white dark:bg-brand-charcoal rounded-3xl p-12 border border-brand-beige/50 dark:border-brand-dark-grey/50 text-center max-w-lg mx-auto shadow-sm">
          ...
          <p className="text-xs text-brand-dark-grey dark:text-brand-gold-light mb-6">
            {search || statusFilter !== "all"
              ...
              : "No late attendance policies have been configured yet."}
          </p>
          {canAdd && (
            <button
              onClick={handleOpenAdd}
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="bg-brand-offwhite dark:bg-brand-midnight uppercase text-[10px] font-black tracking-wider text-brand-dark-grey dark:text-brand-gold-light border-b border-brand-beige/60 dark:border-brand-dark-grey/60">
                    <tr>
                      ...
                      <th className="py-4 px-6">Grace & Tolerance</th>
                      <th className="py-4 px-6">Deduction Rule</th>
                      <th className="py-4 px-6">Absence / Comp.</th>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-4 px-6">
                              ...
                              <div className="text-[10px] text-brand-dark-grey dark:text-brand-gold-light/70 font-semibold mt-0.5 capitalize">
                                Matching: {policy.matchingMethod?.replace("_", " ")} • Present: {policy.presentRule?.replace("_", " ")}
                              </div>
                            </td>
                            <td className="py-4 px-6 text-brand-dark-grey dark:text-brand-gold-light/90 font-medium">
                              <div className="flex flex-col gap-0.5">
                                <span>Grace: {policy.graceBeforeStart}m / {policy.graceAfterEnd}m</span>
                                <span className="text-amber-500 font-bold">Late Tol: {policy.lateTolerance}m</span>
                              </div>
                            </td>
                            <td className="py-4 px-6 text-brand-dark-grey dark:text-brand-gold-light/90 font-medium">
                              <div className="flex flex-col gap-0.5">
                                <span className="uppercase text-brand-red font-bold text-[11px]">
                                  {policy.deductionType?.replace("_", " ")} ({policy.deductionAmount})
                                </span>
                                <span>Max Allowed: {policy.maxLateAllowedPerMonth}/mo</span>
                              </div>
                            </td>
                            <td className="py-4 px-6 text-brand-dark-grey dark:text-brand-gold-light/90 font-medium">
                              <div className="flex flex-col gap-0.5">
                                <span>Count Absent: <b className="uppercase">{policy.lateCountAsAbsent}</b></span>
                                <span>Compensate: <b className="uppercase text-emerald-500">{policy.compensatedLate}</b></span>
                              </div>
                            </td>
                            <td className="py-4 px-6 text-center">
                              <span
                      ...
                      <div>
                        <div className="flex items-start justify-between gap-3 mb-3">
                          <div>
                            ...
                            <span className="text-[10px] text-brand-dark-grey dark:text-brand-gold-light/70 font-semibold block mt-0.5 capitalize">
                              {policy.matchingMethod?.replace("_", " ")} • {policy.presentRule?.replace("_", " ")}
                            </span>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 my-4 text-[11px] font-bold">
                          <div className="p-2.5 rounded-2xl bg-brand-offwhite dark:bg-brand-midnight">
                            <span className="text-[9px] text-brand-dark-grey block uppercase">Grace Start/End</span>
                            <span>{policy.graceBeforeStart}m / {policy.graceAfterEnd}m</span>
                          </div>
                          <div className="p-2.5 rounded-2xl bg-brand-offwhite dark:bg-brand-midnight">
                            <span className="text-[9px] text-brand-dark-grey block uppercase">Tolerance</span>
                            <span className="text-amber-500">{policy.lateTolerance}m</span>
                          </div>
                          <div className="p-2.5 rounded-2xl bg-brand-offwhite dark:bg-brand-midnight">
                            <span className="text-[9px] text-brand-dark-grey block uppercase">Deduction</span>
                            <span className="text-brand-red uppercase truncate block">{policy.deductionType}</span>
                          </div>
                          <div className="p-2.5 rounded-2xl bg-brand-offwhite dark:bg-brand-midnight">
                            <span className="text-[9px] text-brand-dark-grey block uppercase">Max Late / Mo</span>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-brand-dark-grey dark:text-brand-gold-light font-medium">
                      Configure attendance tolerance and deduction parameters
                    </p>
                  </div>
                </div>
                <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      disabled={isSubmitting}
                      onChange={(e) => setFormData((prev) => ({ ...prev, matchingMethod: e.target.value }))}
                    ...
                    >
                      <option value="window">Window Matching (Within Shift)</option>
                      <option value="nearest_shift">Nearest Shift (Closest Start)</option>
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      disabled={isSubmitting}
                      onChange={(e) => setFormData((prev) => ({ ...prev, presentRule: e.target.value }))}
                      className="w-full px-4 py-2.5 rounded-2xl text-xs font-bold bg-brand-offwhite dark:bg-brand-midnight border border-brand-beige/60 dark:border-brand-dark-grey text-brand-black dark:text-brand-white outline-none focus:ring-2 focus:ring-brand-gold/50 cursor-pointer"
                    >
                      <option value="both_punches">Both Punches Required (In & Out)</option>
                      <option value="single_punch">Single Punch Allowed</option>
                      <option value="in_punch_only">In-Punch Only</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      disabled={isSubmitting}
                      onChange={(e) => setFormData((prev) => ({ ...prev, graceBeforeStart: Number(e.target.value) }))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      disabled={isSubmitting}
                      onChange={(e) => setFormData((prev) => ({ ...prev, graceAfterEnd: Number(e.target.value) }))}
                      className="w-full px-4 py-2.5 rounded-2xl text-xs font-bold bg-brand-offwhite dark:bg-brand-midnight border border-brand-beige/60 dark:border-brand-dark-grey text-brand-black dark:text-brand-white outline-none focus:ring-2 focus:ring-brand-gold/50"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      disabled={isSubmitting}
                      onChange={(e) => setFormData((prev) => ({ ...prev, lateTolerance: Number(e.target.value) }))}
                      className="w-full px-4 py-2.5 rounded-2xl text-xs font-bold bg-brand-offwhite dark:bg-brand-midnight border border-brand-beige/60 dark:border-brand-dark-grey text-brand-black dark:text-brand-white outline-none focus:ring-2 focus:ring-brand-gold/50"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      ...
                      <option value="quarter_day">Quarter Day Salary</option>
                  ...
                  <div>
                    <label className="block text-xs font-extrabold text-brand-black dark:text-brand-white uppercase tracking-wider mb-1">
                      Deduction Amount ({currencySymbol})
                    </label>
                    <input
                      type="number"
                      min={0}
                      ...
                      disabled={isSubmitting}
                      onChange={(e) => setFormData((prev) => ({ ...prev, deductionAmount: Number(e.target.value) }))}
                      className="w-full px-4 py-2.5 rounded-2xl text-xs font-bold bg-brand-offwhite dark:bg-brand-midnight border border-brand-beige/60 dark:border-brand-dark-grey text-brand-black dark:text-brand-white outline-none focus:ring-2 focus:ring-brand-gold/50"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      disabled={isSubmitting}
                      onChange={(e) => setFormData((prev) => ({ ...prev, maxLateAllowedPerMonth: Number(e.target.value) }))}
                      className="w-full px-4 py-2.5 rounded-2xl text-xs font-bold bg-brand-offwhite dark:bg-brand-midnight border border-brand-beige/60 dark:border-brand-dark-grey text-brand-black dark:text-brand-white outline-none focus:ring-2 focus:ring-brand-gold/50"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-extrabold text-brand-black dark:text-brand-white uppercase tracking-wider mb-1">
                      Late Count As Absent
                    </label>
                    <select
                      value={formData.lateCountAsAbsent}
                      disabled={isSubmitting}
                      onChange={(e) => setFormData((prev) => ({ ...prev, lateCountAsAbsent: e.target.value }))}
                      className="w-full px-4 py-2.5 rounded-2xl text-xs font-bold bg-brand-offwhite dark:bg-brand-midnight border border-brand-beige/60 dark:border-brand-dark-grey text-brand-black dark:text-brand-white outline-none focus:ring-2 focus:ring-brand-gold/50 cursor-pointer"
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      disabled={isSubmitting}
                      onChange={(e) => setFormData((prev) => ({ ...prev, compensatedLate: e.target.value }))}
                      className="w-full px-4 py-2.5 rounded-2xl text-xs font-bold bg-brand-offwhite dark:bg-brand-midnight border border-brand-beige/60 dark:border-brand-dark-grey text-brand-black dark:text-brand-white outline-none focus:ring-2 focus:ring-brand-gold/50 cursor-pointer"
                    >
                      <option value="yes">Yes</option>
                      <option value="no">No</option>
      ...
      <ConfirmDeleteModal
        ...
        itemName={deletingItem?.policyName || "Late Policy"}
