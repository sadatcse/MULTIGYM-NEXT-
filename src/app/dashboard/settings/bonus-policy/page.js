...
export default function BonusPolicyPage() {
  ...
  const totalCount = stats?.totalPolicies || bonusPolicies.length;
  const activeCount = stats?.activeCount ?? bonusPolicies.filter((p) => p.status === "active").length;
  const inactiveCount = Math.max(0, totalCount - activeCount);
  const uniqueTypesCount = new Set(bonusPolicies.map((p) => p.bonusType).filter(Boolean)).size || 1;
  ...
  const handleOpenEdit = (item) => {
    ...
    setFormData({
      ...
      applicableMonth: item.applicableMonth || "March",
  ...
  const validateForm = () => {
    ...
    const duplicateName = bonusPolicies.find(
      (p) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        ...
        Swal.fire({
          title: "Updated!",
          text: `Bonus policy "${formData.policyName}" updated successfully.`,
          icon: "success",
        ...
        Swal.fire({
          title: "Created!",
          text: `Bonus policy "${formData.policyName}" created successfully.`,
          icon: "success",
          confirmButtonColor: "#FF1818",
          timer: 2000,
        });
      ...
      const msg = err?.response?.data?.message || "Failed to save bonus policy.";
      Swal.fire({
        title: "Error!",
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteBonusPolicy(deletingItem._id);
      Swal.fire({
        title: "Deleted!",
        text: `Bonus policy "${deletingItem.policyName}" has been deleted.`,
        icon: "success",
        confirmButtonColor: "#FF1818",
        timer: 2000,
      });
      ...
      const msg = err?.response?.data?.message || "Failed to delete bonus policy.";
      Swal.fire({
        title: "Error!",
        text: Array.isArray(msg) ? msg.join(", ") : msg,
        icon: "error",
    ...
    return (
      <div className="space-y-6 w-full max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-6 pb-10 font-sans">
        <Mtitle
          ...
          subtitle="Configure festival, performance, and annual bonus calculation methods, disbursement rates, and payout schedules."
        />
        <div className="bg-brand-white dark:bg-brand-charcoal p-8 rounded-3xl border border-brand-beige/50 dark:border-brand-dark-grey/50 text-center max-w-xl mx-auto shadow-xl">
          ...
          <p className="text-xs text-brand-dark-grey dark:text-brand-gold-light mb-6">
            You do not have view permission for Bonus Policy. Please contact your system administrator.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6 w-full max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-6 pb-10 font-sans">
      ...
      <div className="bg-brand-white dark:bg-brand-charcoal p-4 rounded-3xl border border-brand-beige/50 dark:border-brand-dark-grey/50 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            placeholder="Search bonus policy..."
      ...
      {loading ? (
        ...
        <div className="bg-brand-white dark:bg-brand-charcoal rounded-3xl p-12 border border-brand-beige/50 dark:border-brand-dark-grey/50 text-center max-w-lg mx-auto shadow-sm">
          ...
          <h3 className="text-lg font-black text-brand-black dark:text-brand-white mb-1">
            No Bonus Policies Found
          </h3>
          <p className="text-xs text-brand-dark-grey dark:text-brand-gold-light mb-6">
            {search || statusFilter !== "all"
              ? "No policies match your active filters."
              : "No bonus payout policies have been configured yet."}
          </p>
          {canAdd && (
            <button
              onClick={handleOpenAdd}
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="bg-brand-offwhite dark:bg-brand-midnight uppercase text-[10px] font-black tracking-wider text-brand-dark-grey dark:text-brand-gold-light border-b border-brand-beige/60 dark:border-brand-dark-grey/60">
                    <tr>
                      ...
                      <th className="py-4 px-6">Bonus Type</th>
                      <th className="py-4 px-6">Calculation Method</th>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2 my-4 text-[11px] font-bold">
                          <div className="p-2.5 rounded-2xl bg-brand-offwhite dark:bg-brand-midnight">
                            <span className="text-[9px] text-brand-dark-grey block uppercase">Bonus Type</span>
                            <span className="text-brand-gold">{policy.bonusType}</span>
                          </div>
                          <div className="p-2.5 rounded-2xl bg-brand-offwhite dark:bg-brand-midnight">
                            <span className="text-[9px] text-brand-dark-grey block uppercase">Payout Month</span>
                            <span>{policy.applicableMonth || "All Months"}</span>
                          </div>
                          <div className="p-2.5 rounded-2xl bg-brand-offwhite dark:bg-brand-midnight">
                            <span className="text-[9px] text-brand-dark-grey block uppercase">Rate / Amount</span>
                          ...
                          <div className="p-2.5 rounded-2xl bg-brand-offwhite dark:bg-brand-midnight">
                            <span className="text-[9px] text-brand-dark-grey block uppercase">Calculation</span>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-brand-dark-grey dark:text-brand-gold-light font-medium">
                      Configure bonus calculation method, disbursement rate, and schedule
                    </p>
                  </div>
                </div>
                <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      disabled={isSubmitting}
                      onChange={(e) => setFormData((prev) => ({ ...prev, applicableMonth: e.target.value }))}
                      className="w-full px-4 py-2.5 rounded-2xl text-xs font-bold bg-brand-offwhite dark:bg-brand-midnight border border-brand-beige/60 dark:border-brand-dark-grey text-brand-black dark:text-brand-white outline-none focus:ring-2 focus:ring-brand-gold/50 cursor-pointer"
                    >
                      {MONTHS.map((m) => (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      ...
                      <option value="fixed_amount">Fixed Lump Sum ({currencySymbol})</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-xs font-extrabold text-brand-black dark:text-brand-white uppercase tracking-wider mb-1">
                      Rate / Amount ({formData.calculationType === "fixed_amount" ? currencySymbol : "%"})
                    </label>
                    <input
                      type="number"
                      min={0}
                      ...
                      disabled={isSubmitting}
                      onChange={(e) => setFormData((prev) => ({ ...prev, bonusRate: Number(e.target.value) }))}
      ...
      <ConfirmDeleteModal
        ...
        itemName={deletingItem?.policyName || "Bonus Policy"}
