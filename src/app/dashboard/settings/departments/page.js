...
export default function DepartmentsPage() {
  ...
  const canAdd = can("departments", "add");
    ...
    return (
      <div className="space-y-6 w-full max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-6 pb-10">
        <Mtitle
          ...
          subtitle="Manage organizational structure, display hierarchy order, and active department access."
        />
        <div className="bg-brand-white dark:bg-brand-charcoal p-12 rounded-3xl border border-brand-beige/50 dark:border-brand-dark-grey/50 text-center space-y-4 shadow-sm">
          ...
          <p className="text-xs text-brand-dark-grey dark:text-brand-gold-light max-w-md mx-auto leading-relaxed">
            You do not have view permission for Department Configuration. Contact your System Administrator to request access.
          </p>
        </div>
      </div>
    );
  ...
  const validateForm = () => {
    ...
    const dupName = departments.find(
      (d) =>
        d.name.toLowerCase() === trimmedName.toLowerCase() &&
        d._id !== editingDept?._id
    );
    if (dupName) {
      setFormError(`Department "${trimmedName}" already exists on current page.`);
      return false;
    }

    const dupOrder = departments.find(
      (d) => d.order === Number(formData.order) && d._id !== editingDept?._id
    );
    if (dupOrder) {
    ...
    try {
      if (editingDept) {
        ...
        Swal.fire({
          ...
          text: `Department "${formData.name}" has been updated.`,
          icon: "success",
        ...
        Swal.fire({
          ...
          text: `Department "${formData.name}" added to system.`,
          icon: "success",
          confirmButtonColor: "#FF1818",
        });
      }
  ...
  const handleConfirmDelete = async () => {
    if (!deletingDept || isDeleting) return;
    setIsDeleting(true);
    try {
      ...
      Swal.fire({
        title: "Deleted!",
        text: `Department "${deletingDept.name}" has been removed.`,
        icon: "success",
        confirmButtonColor: "#FF1818",
      });
    } catch (err) {
  ...
  return (
    <div className="space-y-6 w-full max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-6 pb-10">
                  ...
                  <thead className="bg-brand-offwhite dark:bg-brand-midnight uppercase text-[10px] font-extrabold tracking-widest text-brand-dark-grey dark:text-brand-gold-light border-b border-brand-beige/60 dark:border-brand-dark-grey/60">
                    <tr>
                      ...
                      <th className="py-4 px-6">
                        <span className="inline-flex items-center gap-1">
                          <FiLayers className="text-brand-gold" /> Department Name
                        </span>
                      </th>
                    ...
                    {departments.length > 0 ? (
                      <AnimatePresence initial={false}>
                        {departments.map((dept, idx) => {
                          const rowBusy = isDeleting && deletingDept?._id === dept._id;
                          return (
                            <motion.tr
                              key={dept._id}
                              custom={idx}
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <span className="text-[10px] text-brand-dark-grey dark:text-brand-gold-light font-medium">
                                      Organizational Division #{dept.order}
                                    </span>
                                  </div>
                                </div>
                              </td>
                                ...
                                >
                                  <span
                                    className={`w-1.5 h-1.5 rounded-full ${
                                      dept.status === "active" ? "bg-emerald-500 animate-pulse" : "bg-brand-red"
                                    }`}
                                  />
                                  {dept.status}
                                </span>
            ...
            >
              ...
              <div className="p-5 bg-brand-offwhite dark:bg-brand-midnight border-b border-brand-beige/50 dark:border-brand-dark-grey/50 flex items-center justify-between">
                <h3 className="text-base font-bold text-brand-black dark:text-brand-white flex items-center gap-2">
                  <FiLayers className="text-brand-gold" />
      ...
      <ConfirmDeleteModal
        ...
        itemName={deletingDept?.name || "Department"}
